...
expName = 'Motor_sequence_task'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\vidna\\OneDrive\\Documents\\11-10-24\\Motor_sequence_task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fixxation = visual.ShapeStim(
        win=win, name='fixxation', vertices='cross',units='pix', 
        size=(10,10),
    ...
    for thisTrial in trials:
        ...
        thisExp.addData('trial_2.started', globalClock.getTime(format='float'))
        probe.setPos((pos, 60))
        ...
        trial_2Components = [fixxation, line_1, line_2, line_3, line_4, probe, key_resp]
        ...
        while continueRoutine:
            ...
            # if fixxation is starting this frame...
            if fixxation.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                fixxation.frameNStart = frameN  # exact frame index
                fixxation.tStart = t  # local t and not account for scr refresh
                fixxation.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(fixxation, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'fixxation.started')
                # update status
                fixxation.status = STARTED
                fixxation.setAutoDraw(True)
            
            # if fixxation is active this frame...
            if fixxation.status == STARTED:
                # update params
                pass
            
            # if fixxation is stopping this frame...
            if fixxation.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixxation.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    fixxation.tStop = t  # not accounting for scr refresh
                    fixxation.tStopRefresh = tThisFlipGlobal  # on global time
                    fixxation.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'fixxation.stopped')
                    # update status
                    fixxation.status = FINISHED
                    fixxation.setAutoDraw(False)
